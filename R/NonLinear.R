...
#################################################################################
#' @export
fait <- function(betak, i, t, A, TCOV, fct){
...
diffaitbeta <- function(betak, i, t, A, TCOV, fct, diffct, diffctind){
  if (diffctind == 0 ){
    ...
    return(jacobian(func = ffh, x = betak, t = A[i,t], TCOV = TCOV))
  }else{
...
gkNL <- function(beta, sigma, i, k, TCOV, A, Y, fct){
  ...
  return(max(prod(dnorm((Y[i, ]-muikt)/sigma[k])/sigma[k]), 10**(-300)))
}
#################################################################################
  ...
  for (l in 1:nbeta[k]){
    ...
    for (i in 1:n){
      ...
      for (t in ind){
        ...
        difbkl = difbkl + der[l]*exp(-2*alpha[k])*m[t]*dnorm(m[t])*prod(exp(-alpha[k])*dnorm(m[-tind]))
        tind = tind +1
      }
      py = prod(exp(-alpha[k])*dnorm(m[ind]))
...
difLsigmakalphaNL <- function(param, k, ng, nx, nbeta, n, A, Y, X, TCOV, fct){
  ...
  for (i in 1:n){
    ...
    tind = 1
    for (t in ind){
      difbkl = difbkl + (-1+m[t]**2)*exp(-alpha[k])*dnorm(m[t])*prod(exp(-alpha[k])*dnorm(m[-tind]))
      tind = tind + 1
    }
    py = prod(exp(-alpha[k])*dnorm(m))
    s = sum(sapply(1:ng, function(s){piik(theta, i, s, ng, X)*gkalphaNL(beta, alpha, i, s, TCOV, A, Y, fct)}))
    a = a + piik(theta, i, k, ng, X)/s*difbkl
  }
  alphas = c(alphas, a)
...
#################################################################################
difLalphaNL <- function(param, ng, nx, nbeta, n, A, Y, X, TCOV, diffctind){
  out =c()
  for (k in 1:ng){
...
#################################################################################
LikelihoodalphaNL <- function(param, ng, nx, nbeta, n, A, Y, X, TCOV, fct){
  theta = param[1:(ng*nx)]
  betatmp = param[((ng*nx)+1):(ng*nx+sum(nbeta))]
  alpha = param[-c(1:(ng*nx+sum(nbeta)))]
  j = 1
...
fctbeta<- function(betak, Y, A, TCOV, zk, period, n, nbetak, diffctind){
  tmp1 = c()
  tmp2 = c()
  tmp3 = c()
  for (i in 1:n){
...
fctbeta.jac <- function(betak, Y, A, TCOV, zk, period, n, nbetak, diffctind){
  tmp1 = c()
  tmp2 = c()
  tmp3 = c()
  for (i in 1:n){
...
EMNL <- function(param, ng, nx, nbeta, n, A, Y, X, TCOV, nw, itermax, EMIRLS, fct, diffct, diffctind, nls.lmiter){
  ...
  while (tour<itermax){
    ...
    # M-step
    for (k in 1:ng){
      betatmp = nls.lm(par = beta[(nbetacum[k]+1):(nbetacum[k+1])], fn = fctbeta, jac = fctbeta.jac,
                       Y = Y, A = A, TCOV = TCOV, zk = zk[ ,k], diffctind = diffctind,
                       period = period, n =n, nbetak = nbeta[k], 
                       control = nls.lm.control(maxiter = nls.lmiter))$par
    ...
    if (nx == 1){
      ...
      newpi = findtheta(pi, taux, X, n, ng, period, EMIRLS)
...
dif2fait <- function(betak, i, t, A, TCOV, fct, diffct, diffctind){
  if (is.null(diffctind == 0)){
    ...
    return(hessian(func = ffh, x = betak, t = A[i,t], TCOV = TCOV))
  }else{
    dffh <- function(beta, t, TCOV){
      return(diffct(t, beta, TCOV))
    }
    return(jacobian(func = dffh, x = betak, t = A[i,t], TCOV = TCOV))
  }
}
#################################################################################
...
#################################################################################
msigmaNL = function(i, t, ng, nbeta, A, Y, beta, sigma, taux, nbetacum, TCOV, period, delta, ndeltacum, nw){
  tmp = c()
  for (k in 1:ng){
