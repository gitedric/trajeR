  ...
  for (int l = 0; l < nbeta[k]; ++l){
    ...
    for (int i = 0; i < n; i++){
      ...
      for (int t = 0; t < period; ++t){
        ...
        tmp2 += pow(A(i, t), l)*exp(Y(i, t)*muikt[t])/pow((1+exp(muikt[t])), 2)*(Y(i, t) - (1-Y(i, t))*exp(muikt[t]))*prodvect(tmp1minus);
      }
      double so = 0;
      for (int s = 0; s < ng; ++s){
...
NumericVector difLLOGIT_cpp(NumericVector param,
  ...
  if (param.length() > ng*nx+sum(nbeta)){
    delta = param[Range(ng*nx+sum(nbeta), param.length())];
    if (nw != 0){
      int ind = 0;
      for (int i = 0; i < ng; i++){
        NumericVector tmp;
...
NumericVector EMLOGIT_cpp(NumericVector param,
  ...
  while (tour < itermax){
    if (nx == 1){
      ...
      // a modifier
      Rprintf("iter %3d value ", tour);
      Rprintf("%.6f\n", -likelihoodLOGIT_cpp(NumericVector(vparam.begin(), vparam.end()), ng, nx, n, nbeta, A, Y, X, TCOV, nw));
    }
    // E-step
        ...
        while(all(abs(precIRLS) > 0.000001) &&  stop < 300){
          ...
          for (int i = 0; i < n; ++i){
            for (int t = 0; t < period; ++t){
              ...
              double deltaWit = sum(deltaIRLS*vtmp3);
              double rhoikt = exp(betaAit)/(1+exp(betaAit));
              for (int kk = 0; kk < nbeta[k]; ++kk){
                Aw(kk, period*i + t) = pow(A(i, t), kk);
              }
          ...
          mat Z = diagmat(tmp2);
          newbetaIRLS = inv(Aw*Z*W*trans(Aw))*Aw*Z*W*S;
          newdeltaIRLS = inv(Ww*Z*W*trans(Ww))*Ww*Z*W*Sw;
