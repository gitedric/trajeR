...
// ----------------------------------------------------------------------------
NumericVector difLsigmaalphaunique_cpp(NumericVector theta,
                                       List beta,
                                       NumericVector alpha,
                                       Nullable<List> delta,
                                       int ng,
...
NumericVector difLalpha_cpp(NumericVector param,
  ...
  NumericVector alpha = param[Range(ng*nx+sum(nbeta), ng*nx+sum(nbeta)+ng-1)];
...
double Likelihoodalpha_cpp(NumericVector param,
  ...
  for (int i =0; i < n; ++i){
    double a = 0;
    for (int s = 0; s < ng; ++s){
      a +=  piikIntern_cpp(theta, i, s, ng, X)*gkalpha_cpp(betaL, alpha, i, s, nbeta, A, Y, ymin, ymax, TCOV, deltaL, nw);
    }
    out += log(a);
  }
  return(out);
